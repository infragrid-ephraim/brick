...
const PRIMITIVE_DOCS: Record<string, string> = {
  ...
  gen: [
    ...
    "```brick",
    "// Function-call style (preferred)",
    "gen(\"Summarize @text\") -> @summary",
    "gen(\"Extract info\", Article) -> @article",
    "gen(\"Find all items\", Article[]) -> @list",
    "gen(\"Detailed analysis\", Article[], \"gpt-4o\") -> @detailed",
    "",
    "// Keyword style (also supported)",
    "gen \"Summarize @text\" -> @summary",
    "gen \"Extract info\" as Article -> @article",
    "gen \"Detailed analysis\" as Article using \"gpt-4o\" -> @detailed",
    "```",
    "Calls the LLM with a prompt. The second argument (type) enforces a JSON Schema — supports object types, enums, arrays, `String`, `Number`, `Boolean`. The third argument (optional) is the model name.",
  ].join("\n"),
  ...
  gen_code: [
    ...
    "gen_code(\"Parse @csvText into rows\", Row[]) -> @rows",
    "gen_code(\"Summarize @data\", String, \"gpt-4o\") -> @summary",
    "```",
